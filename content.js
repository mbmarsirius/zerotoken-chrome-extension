...
// Removed: visual handoff button clone (caused duplication/flicker)
// Removed: visual clone sync (caused duplication/flicker)
// Removed: handoff proxy (caused duplication/flicker)
// Removed: restore visual proxy (not needed)
// Removed: proxy above #zt-hint (not needed)
...
(() => {
  ...
  if (!r) return;
  ['#zt-handoff-proxyDEADDEAD','#zt-handoff-btn-wrap','#zt-handoff-btn-visualDEADDEAD'].forEach(sel=>{
    r.querySelectorAll(sel).forEach(n=>n.remove());
  });
})();
// Removed: extra stable fallback button (not needed)
// Removed: hard reset loop (not needed when proxies are gone)
// Removed: proxy cleaner observer (no longer necessary)
// Removed: killer observer (redundant)
// Removed: duplicate killer observer (redundant)
// Removed: killer v2 observer (redundant)
